...
│   └── SPRINT-7.md           # 순환 참조 탐지 + 코드 복잡도 히트맵
├── backend/
│   ├── main.py               # FastAPI 엔트리
│   ├── cli.py                # CLI 엔트리포인트
│   ├── parser/
...
└── frontend/
    ├── src/
    │   ├── App.tsx
    │   ├── components/       # ProjectInput, SummaryCards, MethodTable, ComplexityMap, etc.
    │   ├── hooks/useApi.ts
    │   └── types/index.ts
    ├── package.json
    └── tailwind.config.js
...
| `SPRINT-1~6.md` | 스프린트별 태스크 체크리스트, DoD, 산출물 | 개발 진행 기록 추적 |
| `SPRINT-7.md` | 순환 참조 탐지 + 코드 복잡도 히트맵 태스크 이력 | 고도화 기능 개발 기록 |
...
| **완성도 및 UX** — 완성도 | 핵심 기능 동작 | Sprint 1~7 DoD, E2E 테스트 |
| **완성도 및 UX** — 사용자 경험 | 자연스러운 흐름, 직관적 UI | `PRD.md` §4 사용자 스토리, §14.6 사용성 테스트 |
...
| 평가 기준 | 대응 위치 |
|----------|-----------|
| 가설 설정 | `PRD.md` §14.1 핵심 가설 (H1~H6) |
| 성공 지표 + 측정 방법 | `PRD.md` §14.2 성공 지표 테이블 |
...
| 사용성 테스트 계획 | `PRD.md` §14.6 사용성 테스트 계획 |
...
| 평가 기준 | 대응 위치 |
|----------|-----------|
| 테스트 전략 — 단위/통합 테스트 존재 및 커버리지 | `PRD.md` §14.4 테스트 전략, `CLAUDE.md` 테스트 컨벤션 |
| CI/CD — 자동화 파이프라인 | `PRD.md` §14.5 CI/CD, `.github/workflows/` |

### QA 고유

| 평가 기준 | 대응 위치 |
|----------|-----------|
| 테스트 전략 — 단위/통합/E2E 체계 | `PRD.md` §14.4 테스트 전략 (3단계 분류) |
| 테스트 커버리지 — 핵심 기능 커버 | `PRD.md` §14.4 커버리지 목표 테이블 |
| 테스트 자동화 — 구현 또는 계획 | `PRD.md` §14.5 CI/CD (GitHub Actions 자동 실행) |

## 현재 진행 상태
- [x] PRD 작성 완료 (문제 정의, 기능 명세, 차별화, 검증 계획 포함)
- [x] 스프린트 문서 분리 완료 (7개 스프린트)
...
- [x] Sprint 7: 순환 참조 탐지 + 코드 복잡도 히트맵 — 완료
